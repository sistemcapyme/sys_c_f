...
import {
  ...
  DollarSign, FileCheck, Shield, AlertTriangle
} from 'lucide-react';
...
const CatalogosAdmin = () => {
  ...
  const handleToggleActivo = (pdf) => {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          // Si el servicio no tiene una ruta toggle específica, usamos actualizarPdf
          await catalogosService.actualizarPdf(pdf.id, { ...pdf, activo: !pdf.activo });
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--gray-50)' }}>
                  {['Título', 'Precio', 'Enlace', 'Estado', 'Acciones'].map((h, i) => (
